...
import { resolveImageUrl, resolveAvatarUrl, buildUiAvatar, FALLBACK_IMAGE, FALLBACK_AVATAR, onErrorSetSrc, getDesignImage } from '../utils/imageUtils';
...
const DisenoDetallePage = () => {
  ...
  const handleLike = () => {
    ...
    disenoService.darLike(id)
      ...
      .catch((err) => {
        const msg = err.response?.data?.message || "No se pudo completar la acción. Intenta más tarde.";
        toast.error(msg);
      });
  };
  ...
  const imagenes = (() => {
    const arr = Array.isArray(diseno?.imagenesUrls) && diseno.imagenesUrls.length
      ? diseno.imagenesUrls
      : (diseno?.imagenUrl ? [diseno.imagenUrl] : []);
    const resolved = arr.map(resolveImageUrl).filter(Boolean);
    // Si no hay imágenes, usar getDesignImage como fallback inteligente
    return resolved.length > 0 ? resolved : [getDesignImage(diseno)];
  })();
  ...
  const handleAddToCart = () => {
    ...
    // Permitir añadir al carrito sin importar si está autenticado o su rol
    // La verificación de CLIENTE se hará en el checkout
    const yaEnCarrito = cart.items.some(item => item.id === diseno.id);
    if (yaEnCarrito) {
      toast.info("Este diseño ya está en tu carrito");
      return;
    }
        ...
        <CardContent className="p-0">
          <div className="grid gap-6 md:grid-cols-2">
                  ...
                  <div className="relative">
                    ...
                    {imagenes.length > 1 && (
                      <div className="absolute inset-0 flex items-center justify-between px-1">
                        <Button variant="outline" size="sm" onClick={() => setActiveIdx((activeIdx - 1 + imagenes.length) % imagenes.length)}>‹</Button>
                        <Button variant="outline" size="sm" onClick={() => setActiveIdx((activeIdx + 1) % imagenes.length)}>›</Button>
                      </div>
                    )}
                  </div>
                  {imagenes.length > 1 && (
            ...
            <div>
              ...
              {/* Botón "Añadir al Carrito" visible para todos */}
              <Button className="w-full" onClick={handleAddToCart}>
                Añadir al Carrito
              </Button>
              {!user || user.rol !== 'CLIENTE' ? (
                <p className="mt-2 text-xs text-slate-500 text-center">
                  Podrás completar la compra iniciando sesión como cliente.
                </p>
              ) : null}
